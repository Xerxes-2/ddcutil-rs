            ...
            if let Ok(info) = info {
                if info.flags.is_readable() {
                    let value = if info.flags.is_non_table() {
                        handle
                            .vcp_get_value(*code)
                            .map(|v| format!("{} / {}", v.value(), v.maximum()))
                    } else {
                    ...
                    println!(
                        "VCP 0x{:02x} = {} - {} [{:?}] {}",
                        code, value, info.name, info.flags, info.description
                    );
                } else {
                    println!(
                        "VCP 0x{:02x} - {} [{:?}] {}",
                        code, info.name, info.flags, info.description
                    );
                }

                if !cap.is_empty() {
                    let values: Vec<_> = cap
                        .iter()
                        .map(|v| (v, info.value_names.get(v)))
                        .map(|(v, name)| {
